...
const core = globalThis.DeliveryCore;
const platform = globalThis.DeliveryPlatform;
if (!core || !platform) throw new Error("共享核心或平台适配器未加载");
const { GRID, FOOD, RECIPES, CATCHABLE, ORDER_POOL, LEVEL_CONFIGS, MAX_LEVEL, WAVES,
  shapeFor, baseIngredients, recipeFor, canFulfill, orderUpgrades, dailyKey, buildDailyConfig } = core;

const PLAYTEST_KEY = "pocket-delivery-playtest-v1";
const playtest = {
  sessionId: platform.randomId(),
...
function loadProgress() {
  try {
    const saved = platform.storage.getJSON("pocket-delivery-progress", {});
...
function saveProgress() {
  try {
    platform.storage.setJSON("pocket-delivery-progress", {
...
function trackEvent(name, details = {}) {
  ...
  try {
    const events = platform.storage.getJSON(PLAYTEST_KEY, []);
    events.push(event);
    platform.storage.setJSON(PLAYTEST_KEY, events.slice(-400));
  } catch (_) { /* Playtest logging must never interrupt the game. */ }
}

function currentSessionEvents() {
  try {
    return platform.storage.getJSON(PLAYTEST_KEY, [])
...
async function sharePlaytestReport() {
  ...
  const result = await platform.share({ title: "外卖小游戏试玩报告", text });
  if (result.status === "copied") showToast("试玩报告已复制，可以直接发给开发者", "good");
  else if (result.status === "failed") showToast("报告生成成功，但当前环境不允许复制", "bad");
}
...
async function shareGame() {
  ...
  const shareResult = await platform.share({ title: "外卖落下来啦！", text, url: platform.currentUrl() });
  if (shareResult.status === "copied") showToast("试玩链接和成绩已复制，可以发给朋友了", "good");
  else if (shareResult.status === "failed") showToast("分享没有成功，可以直接复制浏览器链接", "bad");
}
...
function showBootError(error) {
  ...
  const message = "游戏资源没有加载完整，请刷新页面；如果仍失败，请重新上传全部游戏文件";
  if (toast) {
    toast.textContent = message;
...
function bootGame() {
  try {
    ...
    loader.src = "renderer.js?v=20260720a";
    loader.onload = () => {
      if (typeof window.createCanvasRenderer !== "function") {
...
bootGame();
globalThis.DeliveryWebGameReady = true;
